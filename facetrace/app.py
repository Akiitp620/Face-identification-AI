...
st.title("FaceTrace: Evidence Discovery & Verification Console")
st.caption("Uses genuine Google Lens via SerpApi for OSINT discovery, then ranks visual matches with FaceNet and verifies evidence on-chain.")
...
st.markdown("""
<style>
/* Replace Streamlit's native upload limit text visually */
[data-testid="stFileUploader"] small {
    font-size: 0;
}

[data-testid="stFileUploader"] small::after {
    content: "Limit 500KB per file · JPG, JPEG, PNG, WEBP";
    font-size: 0.875rem;
}
</style>
""", unsafe_allow_html=True)

uploaded_file = st.sidebar.file_uploader("Upload Target Subject Image", type=["jpg", "jpeg", "png", "webp"], help="Maximum 500 KB for web discovery • JPG, JPEG, PNG, WebP")
st.sidebar.caption("Web discovery limit: 500 KB  \nLarger images can still be processed locally, but cannot be submitted for web discovery.")
...
if uploaded_file is not None:
    ...
    is_oversized = file_size_kb > 500
    if is_oversized:
        st.sidebar.error("Image exceeds the 500 KB web-discovery limit.\nPlease choose a smaller image to continue with Google Lens discovery.")
        size_display = f"{file_size_kb/1024:.2f} MB" if file_size_kb >= 1024 else f"{file_size_kb:.2f} KB"
        st.sidebar.error(f"Selected: {size_display} | Maximum: 500 KB")
    ...
    if st.sidebar.button(
        "Run OSINT Discovery Workflow",
        type="primary",
        disabled=not discovery_enabled or is_oversized
    ):
        ...
        if candidates:
            ...
            if best_evidence:
               with st.spinner("Preparing evidence & generating fingerprint..."):
                    try:
                        ...
                        st.info("IPFS unavailable. Evidence integrity will still be verified using its SHA-256 fingerprint.")
...
if st.session_state.stage >= 1:
    ...
    with col1:
        st.image(best.candidate.image_url, caption="Top Visual Match", use_container_width=True)
    with col2:
        st.write("### Top Ranked Evidence")
        st.metric(label="Evidence Match Score", value=f"{best.evidence_match_score:.1f}%")
        st.caption(f"Raw Face Similarity: {best.face_similarity:.4f}")

        domain = urlparse(best.candidate.source_url).netloc
        st.write(f"**Source Domain:** {domain} - [Open source ↗]({best.candidate.source_url})")
    ...
    st.write("Canonical Evidence JSON Structure:")
    ...
    if st.session_state.tx_hash is None:
        ...
        st.code(f"TX HASH: {st.session_state.tx_hash}", language="text")
        ...
        try:
            ...
            with c2:
                st.write("**Scenario B: Simulate Evidence Tampering**")
                st.caption("Simulating evidence tampering by altering the image binary...")
            ...
            has_ipfs = bool(ev.ipfs_cid and ev.ipfs_cid != "unavailable")
            if has_ipfs:
                dot.node('E', 'IPFS CID')
    ...
    if uploaded_file is not None and not is_oversized:
